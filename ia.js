...
IA.tdNumber = (function () {
  function init() {
    ...
    document.querySelectorAll(IA.SELECTOR.tbody_tr).forEach(function (element) {
      ...
      if (
        element.hidden != true &&
        element.closest(IA.SELECTOR.section).hidden != true
      ) {
...
IA.tdNote = (function () {
  ...
  function thMoreBtnClick(element) {
    let _this = element;
    let this_tdNotes = _this
      .closest(IA.SELECTOR.table)
      .querySelectorAll(IA.SELECTOR.td_note);
  ...
  function thMoreBtnChage(element) {
    ...
    let this_tdNotes = _this
      .closest(IA.SELECTOR.table)
      .querySelectorAll(IA.SELECTOR.td_note);
    let this_thNote = _this
      .closest(IA.SELECTOR.table)
      .querySelector(IA.SELECTOR.th_note);
...
IA.infoCount = (function () {
  ...
  function filterPage() {
    ...
    document
      .querySelectorAll('[data-filter-count-state]')
...
IA.filter = (function () {
  ...
  function change() {
    ...
    let filter_author_text =
      filter_author.options[filter_author_value].text.trim();
    let filter_state_text =
      filter_state.options[filter_state_value].text.trim();

    document.querySelectorAll(IA.SELECTOR.tbody_tr).forEach(function (element) {
      let td_author_text = element
        .querySelector(IA.SELECTOR.td_author)
        .innerText.trim();
      let td_state_text = element
        .querySelector(IA.SELECTOR.td_state)
        .innerText.trim();

      if (
        (td_author_text == filter_author_text || filter_author_value == 0) &&
        (td_state_text == filter_state_text || filter_state_value == 0)
      ) {
  ...
  function _setValData() {
    if (document.querySelectorAll(IA.SELECTOR.header).length == 0) {
      ...
      header_height = document
        .querySelector(IA.SELECTOR.header)
  ...
  function init() {
    ...
    if (IA.PAGEOPTION.theme) {
      if (theme.querySelector('option[value="' + IA.PAGEOPTION.theme + '"]')) {
        theme.querySelector(
          'option[value="' + IA.PAGEOPTION.theme + '"]'
        ).selected = true;
      }
      change();
    }
...
IA.sectionNav = (function () {
  function init() {
    ...
    if (
      IA.PAGEOPTION.section &&
      document.querySelectorAll(IA.SELECTOR.nav_btn)[IA.PAGEOPTION.section]
    ) {
      navBtnClick(
        document.querySelectorAll(IA.SELECTOR.nav_btn)[IA.PAGEOPTION.section]
      );
    ...
    document
      .querySelectorAll(IA.SELECTOR.nav_btn)
      .forEach(function (nav_btn, i) {
  ...
  function createNav() {
    document
      ...
      .insertAdjacentHTML(
        'beforeend',
        '<div class="ia-section-nav"><ul><li class="is-active"><a href="javascript:void(0);" data-section-num="0">전체 (' +
          IA.COUNT.total +
          ')</li></ul></div>'
      );

    document
      .querySelectorAll(IA.SELECTOR.section)
      .forEach(function (section, i) {
        let section_title = section.querySelector(
          IA.SELECTOR.section_title
        ).innerText;
        let section_count = section.querySelector(IA.SELECTOR.section_title)
          .dataset.count;
        let section_num = i + 1;

        document
          .querySelector(IA.SELECTOR.nav_ul)
          .insertAdjacentHTML(
            'beforeend',
            '<li><a href="javascript:void(0);" data-section-num=' +
              section_num +
              '>' +
              section_title +
              ' (' +
  ...
  function navBtnClick(element) {
    ...
    document.querySelectorAll(IA.SELECTOR.section).forEach(function (section) {
      if (
        section.dataset.sectionNum == _this_section_num ||
        _this_section_num == 0
      ) {
